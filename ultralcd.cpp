...
static millis_t sd_remove_time = 0;
static bool sd_remove_wait = false;
static bool sd_startup_checked = false;
...
if (!sd_startup_checked) {
  sd_startup_checked = true;

  if (IS_SD_INSERTED && lcd_detected()) {
    card.initsd();
    lcd_sd_status = true;
  }
}

// ====== КАРТА ВСТАВЛЕНА (сразу) ======

if (sd_status) {

  sd_remove_wait = false;  // отменяем ожидание удаления

  if (!lcd_sd_status && lcd_detected()) {
...
// ====== КАРТА ВЫТАЩЕНА (с задержкой) ======
else {

  if (lcd_sd_status && !sd_remove_wait) {
    sd_remove_time = millis();
    sd_remove_wait = true;
  }

  if (sd_remove_wait && ELAPSED(millis(), sd_remove_time + 1000)) {

    // ещё раз проверяем — карта действительно отсутствует
    if (!IS_SD_INSERTED && lcd_detected()) {
